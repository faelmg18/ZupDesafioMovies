...
public abstract class WebServiceComun {
    ...
    private String baseURL = ApplicationContext.BASE_URL;
    ...
    public <T> List<T> sendDataGetList(String url, Class<T> targetClass) throws HttpConnectionException, IOException {
        String response = synchronousHttpConnection.get(url);
        Log.i("Resposta Servidor", response);
       /* T t = getSerializer().toObject(response, targetClass);*/
        List<T> list = null;

        list = getSerializer().objectMapper.readValue(response,
